...
$(document).ready(function () {
  (function () {
    ...
    const PROGRESS_COLOR = getComputedStyle(
      document.documentElement
    ).getPropertyValue("--progress-bg-color");
    ...
    let url = "http:127.0.0.1:8000/api/song";
    ...
    function handleData() {
      /* UPDATE DATA */
      const generaleData = JSON.parse(
        window.localStorage.getItem("generaleData")
      );
      const notificationData = JSON.parse(
        window.localStorage.getItem("notificationData")
      );
      const playlistData = JSON.parse(
        window.localStorage.getItem("playListData")
      );
      ...
      $(window).bind("beforeunload", function () {
        ...
        const playListSaveData = playLists.map(
        ...
        window.localStorage.setItem(
          "generaleData",
          JSON.stringify(generaleSaveData)
        );
        window.localStorage.setItem(
          "playListData",
          JSON.stringify(playListSaveData)
        );
        window.localStorage.setItem(
          "notificationData",
    ...
    function handleEvent() {
      ...
      $changePlayList.click(function () {
        const isCurrentPlaylist =
          $(this).attr("data-position") === currentPLayList;
      ...
      setTimeout(() => {
        $(".notification", $notificationsBox).each((index, notification) => {
          ...
          $(notification).click(() =>
            $(notification).removeClass("active").hide(150)
          );
      ...
      $timeProgress.on("input", function () {
        seekTime =
          ($(this).val() / $(this).attr("max")) * $audio.get(0).duration;
      ...
      $searchInput.on("keyup", function () {
        ...
        $(".info__title, .info__author", searchBox).filter(function () {
          ...
          const isContainText =
            ...
            convertedText.indexOf(searchValue) > -1;
    ...
    function loadCurrentSong() {
      ...
      const isPlayedAllSongs =
        playList.playedSongs.length === playList.songs.length;
      if (isPlayedAllSongs) {
      ...
      $audio.on("loadedmetadata", () => {
        const songDurationTime = customFunctions.formatTime(
